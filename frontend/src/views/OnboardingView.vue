...
const submitPlacement = async () => {
  if (
    Object.keys(placementAnswers.value).length < placementQuestions.value.length
  ) {
  ...
  try {
    const { data } = await api.post("/learning/placement/submit/", {
      answers: placementAnswers.value,
    });
    placementResult.value = data;
    hasExistingResult.value = true;
  } catch (e) {
    showToast("제출 실패: " + (e.response?.data?.error || e.message, "error"));
  }
...
const levelBadge = computed(() => {
  ...
  const m = {
    1: { label: "Level 1 — 기초", color: "#43e97b", emoji: "🌱" },
  ...
  try {
    ...
    if (goalData?.has_goal && goalData.career_goal) {
      myGoal.value = {
        ...
        required_skills: (goalData.career_goal.required_skills || []).map(
...
const setGoal = async (careerGoalId) => {
  try {
    const { data } = await api.post("/learning/goals/set/", {
...
const createCustomGoal = async () => {
  ...
  try {
    const payload = {
      ...
      skills: customGoalForm.value.skills
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean),
    };
...
const gapStats = computed(() => {
  if (!gapMap.value?.skills)
    return { owned: 0, learning: 0, gap: 0, total: 0, rate: 0 };
  ...
  return {
    ...
    gap,
    total,
    rate: total > 0 ? Math.round((owned / total) * 100) : 0,
  };
});
...
const fetchCertificate = async (lectureId) => {
  ...
  try {
    ...
    certData.value = null;
    showToast(e.response?.data?.error || '수료증 정보를 찾을 수 없습니다.', 'error');
  }
};
...
const printCertificate = async () => {
  ...
  try {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { margin: 0; background: #333; display: flex; justify-content: center; align-items: center; padding: 20px; min-height: 100vh; box-sizing: border-box; }
            img { max-width: 100%; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
          </style>
        </head>
        <body>
    ...
    showToast("인쇄 화면을 여는 데 실패했습니다.", "error");
  }
};
...
const downloadImage = async () => {
  ...
  try {
    ...
    link.download = `ReBoot_Certificate_${certData.value.student_name}.png`;
    link.href = canvas.toDataURL("image/png");
    link.click();
    showToast("이미지가 저장되었습니다! 📸 인스타에 자랑해보세요!", "success");
  } catch (e) {
...
<template>
  <div class="onboarding-view">
    ...
    <nav class="ob-tabs">
      ...
      <button
        :class="{ active: activeTab === 'gapmap' }"
        @click="switchTab('gapmap')"
      >
        🗺️ 갭 맵
      </button>
      <button
    ...
    <div class="ob-content">
      ...
      <div v-if="activeTab === 'placement'" class="tab-panel">
        <!-- 결과 표시 -->
        <div
          v-if="hasExistingResult && placementResult"
          class="placement-result glass-panel"
        >
          <div class="result-header">
            ...
            <div>
              ...
              <p>
                점수: <strong>{{ placementResult.score }}</strong> /
                {{ placementResult.total }}
              </p>
            </div>
          </div>
          <div class="result-details" v-if="placementResult.category_scores">
            <div
              v-for="(score, cat) in placementResult.category_scores"
              :key="cat"
              class="cat-score"
            >
              ...
              <div class="cat-bar">
                <div
                  class="cat-fill"
                  :style="{ width: score + '%', background: levelBadge.color }"
                ></div>
              </div>
              <span class="cat-pct">{{ score }}%</span>
            </div>
          </div>
          <button class="btn-outline" @click="retakePlacement">
            다시 진단 받기
          </button>
        </div>
        ...
        <div v-else>
          <div
            class="quiz-intro glass-panel"
            v-if="placementQuestions.length > 0"
          >
            <h2>📋 수준 진단 테스트</h2>
            <p>{{ placementQuestions.length }}문항 · 약 5분 소요</p>
          </div>
          <div
            v-for="q in placementQuestions"
            :key="q.id"
            class="quiz-card glass-panel"
          >
            ...
            <div class="options">
              <label
                ...
                :key="idx"
                class="option-label"
                :class="{ selected: placementAnswers[q.id] === opt }"
              >
                <input
                  type="radio"
                  :name="'pq-' + q.id"
                  :value="opt"
                  v-model="placementAnswers[q.id]"
                />
                {{ opt }}
              </label>
            </div>
      ...
      <div v-if="activeTab === 'goal'" class="tab-panel">
          ...
          <div class="goal-card-main">
            ...
            <div>
              ...
              <div class="goal-meta">
                ...
                <span v-if="myGoal.required_skills?.length"
                  >🔧
                  {{
                    myGoal.required_skills
        ...
        <div class="career-grid">
          ...
          >
            ...
            <div class="career-skills" v-if="career.required_skills">
              <span
                v-for="s in career.required_skills.slice(0, 4)"
                :key="s.id || s"
                class="skill-chip"
                >{{ typeof s === "string" ? s : s.name }}</span
              >
            </div>
            <button class="btn-sm select-btn" @click="setGoal(career.id)">
              선택
            </button>
          </div>
        </div>
        ...
        <div class="custom-section">
          ...
          <div v-if="showCustomGoal" class="custom-form glass-panel">
            <input
              v-model="customGoalForm.title"
              placeholder="직무명 (예: AI 엔지니어)"
            />
            <textarea
              v-model="customGoalForm.description"
              placeholder="설명"
              rows="2"
            ></textarea>
            <input
              v-model="customGoalForm.skills"
              placeholder="필요 스킬 (쉼표 구분)"
            />
            <input
              v-model.number="customGoalForm.estimated_weeks"
              type="number"
              placeholder="예상 기간(주)"
            />
            <button class="btn-primary" @click="createCustomGoal">
              생성 & 설정
            </button>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'gapmap'" class="tab-panel">
        <div v-if="gapMap" class="gapmap-section">
          <div class="gapmap-stats glass-panel">
            <h2>🗺️ 나의 갭 맵</h2>
            <div class="stat-row">
              <div class="gm-stat owned">
                <span>{{ gapStats.owned }}</span> 보유
              </div>
              <div class="gm-stat learning">
                <span>{{ gapStats.learning }}</span> 학습중
              </div>
              <div class="gm-stat gap">
                <span>{{ gapStats.gap }}</span> 미보유
              </div>
              <div class="gm-stat rate">
                <span>{{ gapStats.rate }}%</span> 달성률
              </div>
            </div>
          </div>
          ...
          <div class="skill-list">
            <div
              v-for="skill in gapMap.skills"
              :key="skill.skill_id || skill.name"
              class="skill-item glass-panel"
              :class="skill.status.toLowerCase()"
            >
        ...
        <div v-else class="empty-msg glass-panel">
          ...
          <button class="btn-primary" @click="switchTab('goal')">
            목표 설정하러 가기 →
          </button>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'checklist'" class="tab-panel">
        <div class="checklist-header glass-panel">
          ...
          <div class="cl-actions">
            <button class="btn-outline" @click="analyzeChecklist">
              📊 달성률 분석
            </button>
            <button class="btn-outline danger" @click="getRecoveryPlan">
              🚑 복구 플랜
            </button>
          </div>
        </div>
        ...
        <div v-if="analysisResult" class="analysis-result glass-panel">
          ...
          <p>
            ...
            <strong
              >{{
                analysisResult.completion_rate || analysisResult.rate
              }}%</strong
            >
          </p>
          <p
            v-if="analysisResult.status"
            :class="'status-' + analysisResult.status.toLowerCase()"
          >
        ...
        <div v-if="recoveryPlan" class="recovery-plan glass-panel">
          ...
          <div
            ...
            v-html="recoveryPlan.recovery_plan || recoveryPlan.plan || recoveryPlan.content || ''"
          ></div>
        </div>
        ...
        <div class="cl-items">
          ...
          >
            ...
            <div>
              <span class="cl-text">{{
                item.content || item.objective_content
              }}</span>
      ...
      <div v-if="activeTab === 'certificate'" class="tab-panel">
        <div class="cert-section glass-panel">
          <h2>🏅 수료증 발급</h2>
          <p class="cert-desc">
            수강 중인 강의를 선택하여 수료 자격을 확인하세요.
          </p>
          <div class="cert-input">
            <input
              v-model="certLectureId"
              placeholder="수료증 코드 (6자리)"
              type="text"
              maxlength="6"
              @input="
                certLectureId = $event.target.value
                  .toUpperCase()
                  .replace(/[^A-Z0-9]/g, '')
              "
            />
            <button
              class="btn-primary"
              @click="fetchCertificate(certLectureId)"
            >
              조회
            </button>
          </div>
          ...
          <div v-if="certData" class="cert-result">
            <div v-if="certData.eligible" class="cert-card-container">
              <div class="cert-actions">
                <button class="btn-primary print-btn" @click="printCertificate">🖨️ 수료증 인쇄</button>
                <button class="btn-primary share-btn" @click="downloadImage">📸 인스타에 공유 (이미지 저장)</button>
              </div>
              <div id="printable-certificate" class="printable-cert">
                <div class="cert-inner-border">
                  <div class="cert-header">
                    <h2>수 료 증</h2>
                    <p class="cert-eng-title">Certificate of Completion</p>
                  </div>
                  
                  <div class="cert-body">
                    <div class="cert-row">
                      <span class="cert-label">성 명:</span>
                      <span class="cert-value">{{ certData.student_name }}</span>
                    </div>
                    <div class="cert-row">
                      <span class="cert-label">과 정 명:</span>
                      <span class="cert-value">{{ certData.lecture_title }}</span>
                    </div>
                    <div class="cert-row" v-if="certData.start_date && certData.end_date">
                      <span class="cert-label">기 간:</span>
                      <span class="cert-value">{{ certData.start_date }} ~ {{ certData.end_date }}</span>
                    </div>
                    <div class="cert-row">
                      <span class="cert-label">수료율:</span>
                      <span class="cert-value">{{ certData.completion_rate }}% (총 {{ certData.total_hours }}시간 수료)</span>
                    </div>
                    
                    <p class="cert-statement">
                      위 사람은 Re:Boot Career Build-up Platform에서 주관하는<br/>
                      상기 교육과정을 성실히 이수하였으므로 이 증서를 수여합니다.
                    </p>
                  </div>
                  
                  <div class="cert-footer">
                    <p class="cert-date">{{ certData.issued_date }}</p>
                    <div class="cert-signature">
                      <p>지도교수: {{ certData.instructor_name }}</p>
                      <div class="cert-seal">Re:Boot<br/>직인</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-else class="cert-card not-eligible">
              ...
              <p v-if="certData.remaining">
                남은 항목: {{ certData.remaining }}건
              </p>
            </div>
          </div>
        </div>
...
.print-btn {
  background: linear-gradient(135deg, #11998e, #38f9d7);
  color: #000;
}
.share-btn {
...
.printable-cert {
  ...
  font-family: 'Nanum Myeongjo', 'Gowun Dodum', serif; 
}

.cert-inner-border {
  border: 10px double #4facfe;
  padding: 40px;
  height: 100%;
  position: relative;
}
