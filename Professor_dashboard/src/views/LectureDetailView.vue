...
// Tab Management
const activeTab = ref('monitor'); // 'monitor' | 'attendance' | 'quiz' | 'recording' | 'live' | 'diagnostic' | 'review' | 'material-gen'
...
const switchTab = (tab) => {
    ...
    if (tab === 'material-gen') fetchLiveSessionsForMaterial();
};

// ── STT 기반 강의 자료 생성 ──
const matGenMode = ref('record'); // 'record' | 'session'
const matGenSessions = ref([]);
const matGenSessionsLoading = ref(false);
const matGenSelectedSession = ref(null);
const matGenTranscript = ref('');
const matGenTranscriptLoading = ref(false);
const matGenChunkCount = ref(0);
const matGenType = ref('summary');
const matGenCustomPrompt = ref('');
const matGenGenerating = ref(false);
const matGenResult = ref(null);
const matGenHistory = ref([]);

// 직접 녹음 모드
const matGenRecording = ref(false);
const matGenRecordTime = ref(0);
let matGenRecognition = null;
let matGenRecordTimer = null;
...
const startRecording = () => {
    ...
    matGenRecognition = new SpeechRecognition();
    matGenRecognition.lang = 'ko-KR';
    matGenRecognition.continuous = true;
    matGenRecognition.interimResults = true;
    matGenRecognition.maxAlternatives = 1;

    let finalTranscript = matGenTranscript.value;

    matGenRecognition.onresult = (event) => {
        let interimTranscript = '';
        for (let i = event.resultIndex; i < event.results.length; i++) {
            const result = event.results[i];
            if (result.isFinal) {
                finalTranscript += result[0].transcript + ' ';
                matGenTranscript.value = finalTranscript;
            } else {
                interimTranscript += result[0].transcript;
            }
        }
        // 중간 결과 포함해서 보여줌
        matGenTranscript.value = finalTranscript + interimTranscript;
    };

    matGenRecognition.onerror = (event) => {
        if (event.error === 'no-speech') return; // 무시
    ...
    matGenRecognition.onend = () => {
        // 녹음 중이면 자동 재시작 (continuous 모드 보완)
        if (matGenRecording.value) {
            try { matGenRecognition.start(); } catch(e) { /* 무시 */ }
        }
    };

    matGenRecognition.start();
    matGenRecording.value = true;
    matGenRecordTime.value = 0;
    matGenResult.value = null;

    matGenRecordTimer = setInterval(() => {
        matGenRecordTime.value++;
    }, 1000);
};

const stopRecording = () => {
    matGenRecording.value = false;
    if (matGenRecognition) {
        matGenRecognition.onend = null; // 자동 재시작 방지
        matGenRecognition.stop();
        matGenRecognition = null;
    }
    if (matGenRecordTimer) {
        clearInterval(matGenRecordTimer);
        matGenRecordTimer = null;
    }
};
...
const clearRecording = () => {
    stopRecording();
    matGenTranscript.value = '';
    matGenRecordTime.value = 0;
    matGenResult.value = null;
};

const fetchLiveSessionsForMaterial = async () => {
    matGenSessionsLoading.value = true;
    try {
        const { data } = await api.get(`/learning/live/active/`, { params: { lecture_id: lectureId } });
        matGenSessions.value = Array.isArray(data) ? data : (data.sessions || [data]);
    } catch (e) {
        matGenSessions.value = [];
    }
    try {
        const { data: listData } = await api.get(`/learning/live/`, { params: { lecture_id: lectureId } });
        if (Array.isArray(listData)) {
            matGenSessions.value = listData;
        } else if (listData.results) {
            matGenSessions.value = listData.results;
        }
    } catch (e) { /* 기존 목록 유지 */ }
    matGenSessionsLoading.value = false;
};

const loadTranscript = async (sessionId) => {
    matGenSelectedSession.value = sessionId;
    matGenTranscriptLoading.value = true;
    matGenTranscript.value = '';
    matGenResult.value = null;
    try {
        const { data } = await api.get(`/learning/live/${sessionId}/stt-transcript/`);
        matGenTranscript.value = data.transcript || '';
        matGenChunkCount.value = data.chunk_count || 0;
    } catch (e) {
        showToast('녹취록 로드 실패: ' + (e.response?.data?.error || ''), 'error');
    }
    matGenTranscriptLoading.value = false;
};

const generateMaterial = async () => {
    if (!matGenTranscript.value.trim()) {
        showToast('녹취록이 없습니다. 먼저 녹음하거나 세션을 선택해주세요.', 'warning');
        return;
    }
    // 녹음 중이면 자동 정지
    if (matGenRecording.value) stopRecording();

    matGenGenerating.value = true;
    matGenResult.value = null;
    try {
        let data;
        if (matGenMode.value === 'session' && matGenSelectedSession.value) {
            // 세션 기반 API
            const res = await api.post(`/learning/live/${matGenSelectedSession.value}/generate-material/`, {
                material_type: matGenType.value,
                custom_prompt: matGenCustomPrompt.value,
            });
            data = res.data;
        } else {
            // 직접 녹음 텍스트 기반 API
            const res = await api.post(`/learning/materials/generate-from-text/`, {
                lecture_id: lectureId,
                transcript: matGenTranscript.value,
                material_type: matGenType.value,
                custom_prompt: matGenCustomPrompt.value,
            });
            data = res.data;
        }
        matGenResult.value = data;
        matGenHistory.value.unshift(data);
        showToast(`✅ "${data.title}" 자료가 생성되었습니다!`, 'success');
    } catch (e) {
        showToast('자료 생성 실패: ' + (e.response?.data?.error || ''), 'error');
    }
    matGenGenerating.value = false;
};

const copyGeneratedContent = async () => {
    if (!matGenResult.value?.content) return;
    try {
        await navigator.clipboard.writeText(matGenResult.value.content);
        showToast('📋 마크다운 내용이 복사되었습니다.', 'success');
    } catch {
...
const dismissQuizSuggestion = async () => {
    ...
    quizSuggestion.value = null;
    // 퀴즈 제안 무시 시에는 STT를 중단하지 않음 (교수자는 계속 강의 중)
};
...
<template>
    <div class="detail-view">
        ...
        <div class="tab-nav">
            ...
            <button 
                :class="['tab-btn', { active: activeTab === 'material-gen' }]" 
                @click="switchTab('material-gen')">
                📄 자료 생성
            </button>
        </div>
            ...
            <div v-else class="live-active-section">
                ...
                <div class="live-controls">
                    ...
                    <button v-if="liveSession.status === 'LIVE' && !sttActive"
                            class="btn-stt-start" @click="startSTT">
                        🎙️ STT 시작
                    </button>
                    <button v-if="liveSession.status === 'LIVE' && sttActive"
                            class="btn-stt-stop" @click="stopSTT">
                        🔴 STT 인식 중...
                    </button>
                </div>
                ...
                <div v-if="sttActive" class="stt-preview">
                    <span class="stt-label">🎙️ STT:</span>
        ...
        <!-- Tab: STT 기반 자료 생성 -->
        <!-- ══════════════════════════════════════ -->
        <div v-if="activeTab === 'material-gen'" class="material-gen-tab-content">
            <h2>📄 STT 기반 강의 자료 생성</h2>
            <p class="tab-desc">마이크로 직접 말하거나, 기존 라이브 세션 녹취록으로 AI가 강의 자료를 생성합니다.</p>

            <!-- 모드 전환 -->
            <div class="mg-mode-toggle">
                <button :class="['mg-mode-btn', { active: matGenMode === 'record' }]" @click="matGenMode = 'record'">
                    🎙️ 직접 녹음
                </button>
                <button :class="['mg-mode-btn', { active: matGenMode === 'session' }]" @click="matGenMode = 'session'; fetchLiveSessionsForMaterial()">
                    📂 세션 녹취록
                </button>
            </div>

            <!-- ═══ 모드 A: 직접 녹음 ═══ -->
            <div v-if="matGenMode === 'record'" class="mg-section">
                <h3 class="mg-step-title">① 마이크 녹음</h3>
                <div class="mg-record-area">
                    <div class="mg-record-controls">
                        <button v-if="!matGenRecording" class="mg-mic-btn" @click="startRecording">
                            <span class="mg-mic-icon">🎙️</span>
                            <span>녹음 시작</span>
                        </button>
                        <button v-else class="mg-mic-btn recording" @click="stopRecording">
                            <span class="mg-mic-pulse"></span>
                            <span class="mg-mic-icon">⏹️</span>
                            <span>녹음 중지</span>
                        </button>
                        <span class="mg-record-timer" v-if="matGenRecording || matGenRecordTime > 0">
                            {{ formatRecordTime(matGenRecordTime) }}
                        </span>
                        <button v-if="matGenTranscript && !matGenRecording" class="mg-clear-btn" @click="clearRecording">
                            🗑️ 초기화
                        </button>
                    </div>
                    <div v-if="matGenRecording" class="mg-recording-indicator">
                        <div class="mg-wave-bars">
                            <span></span><span></span><span></span><span></span><span></span>
                        </div>
                        <span class="mg-recording-label">음성을 인식하고 있습니다...</span>
                    </div>
                </div>

                <!-- 실시간 녹취 텍스트 -->
                <div v-if="matGenTranscript" class="mg-transcript-box" style="margin-top: 14px;">
                    <div class="mg-transcript-header">
                        <span>🎙️ 녹취 내용</span>
                        <span class="mg-transcript-len">{{ matGenTranscript.length.toLocaleString() }}자</span>
                    </div>
                    <pre class="mg-transcript-text">{{ matGenTranscript }}</pre>
                </div>
                <div v-else-if="!matGenRecording" class="mg-record-hint">
                    💡 녹음 시작 버튼을 누르고 강의 내용을 말씀해주세요. 음성이 실시간으로 텍스트로 변환됩니다.
                </div>
            </div>

            <!-- ═══ 모드 B: 세션 녹취록 ═══ -->
            <template v-if="matGenMode === 'session'">
                <div class="mg-section">
                    <h3 class="mg-step-title">① 라이브 세션 선택</h3>
                    <div v-if="matGenSessionsLoading" class="loading-state"><div class="spinner"></div>세션 로딩 중...</div>
                    <div v-else-if="matGenSessions.length === 0" class="empty-state">
                        <p>📢 라이브 세션 기록이 없습니다. 먼저 라이브 세션을 진행해주세요.</p>
                    </div>
                    <div v-else class="mg-session-list">
                        <div v-for="s in matGenSessions" :key="s.id"
                             :class="['mg-session-card', { selected: matGenSelectedSession === s.id }]"
                             @click="loadTranscript(s.id)">
                            <div class="mg-session-info">
                                <span class="mg-session-title">{{ s.title || '라이브 세션' }}</span>
                                <span class="mg-session-meta">
                                    <span :class="['mg-status', s.status?.toLowerCase()]">{{ s.status === 'ENDED' ? '종료' : s.status === 'LIVE' ? '진행중' : '대기' }}</span>
                                    · {{ s.session_code }}
                                </span>
                            </div>
                            <span class="mg-session-date">{{ new Date(s.created_at).toLocaleDateString('ko-KR') }}</span>
                        </div>
                    </div>
                </div>

                <div v-if="matGenSelectedSession" class="mg-section">
                    <h3 class="mg-step-title">② STT 녹취록 미리보기</h3>
                    <div v-if="matGenTranscriptLoading" class="loading-state"><div class="spinner"></div>녹취록 로딩 중...</div>
                    <div v-else-if="!matGenTranscript" class="empty-state">
                        <p>⚠️ 이 세션에는 STT 녹취록이 없습니다.</p>
                    </div>
                    <div v-else class="mg-transcript-box">
                        <div class="mg-transcript-header">
                            <span>🎙️ 녹취록 ({{ matGenChunkCount }}개 청크)</span>
                            <span class="mg-transcript-len">{{ matGenTranscript.length.toLocaleString() }}자</span>
                        </div>
                        <pre class="mg-transcript-text">{{ matGenTranscript.substring(0, 2000) }}{{ matGenTranscript.length > 2000 ? '\n\n... (이하 생략)' : '' }}</pre>
                    </div>
                </div>
            </template>

            <!-- ═══ 공통: 자료 유형 선택 + 생성 ═══ -->
            <div v-if="matGenTranscript" class="mg-section">
                <h3 class="mg-step-title">{{ matGenMode === 'record' ? '②' : '③' }} 자료 유형 선택 및 생성</h3>
                <div class="mg-type-selector">
                    <button :class="['mg-type-btn', { active: matGenType === 'summary' }]" @click="matGenType = 'summary'">
                        📝 강의 요약 노트
                    </button>
                    <button :class="['mg-type-btn', { active: matGenType === 'handout' }]" @click="matGenType = 'handout'">
                        📖 학습용 핸드아웃
                    </button>
                    <button :class="['mg-type-btn', { active: matGenType === 'review' }]" @click="matGenType = 'review'">
                        📚 복습 가이드
                    </button>
                </div>

                <div class="mg-type-desc">
                    <template v-if="matGenType === 'summary'">📝 강의 개요, 핵심 개념, 키워드, 수업 흐름, 핵심 정리를 포함한 체계적인 요약 노트</template>
                    <template v-if="matGenType === 'handout'">📖 학습 목표, 핵심 내용, 용어 사전, 연습 문제, 추가 학습 가이드를 포함한 학생 배포용 자료</template>
                    <template v-if="matGenType === 'review'">📚 체크리스트, 개념 비교, 예상 문제, 한 줄 요약, 오답 포인트를 포함한 시험/복습용 가이드</template>
                </div>

                <div class="mg-custom-prompt">
                    <label>💬 추가 지시사항 (선택)</label>
                    <textarea v-model="matGenCustomPrompt" placeholder="예: '데이터베이스 정규화' 부분을 중점적으로 다루어주세요" rows="2"></textarea>
                </div>

                <button class="mg-generate-btn" @click="generateMaterial" :disabled="matGenGenerating">
                    <template v-if="matGenGenerating">
                        <div class="spinner-sm"></div> AI 생성 중... (약 30초 소요)
                    </template>
                    <template v-else>
                        ✨ AI 자료 생성하기
                    </template>
            ...
            <!-- 생성 결과 -->
            <div v-if="matGenResult" class="mg-section mg-result">
                <h3 class="mg-step-title">{{ matGenMode === 'record' ? '③' : '④' }} 생성 결과</h3>
                <div class="mg-result-header">
                    <span class="mg-result-title">{{ matGenResult.title }}</span>
                    <span class="mg-result-badge">{{ matGenResult.material_type }}</span>
                </div>
                <div class="mg-result-content" v-html="renderMarkdown(matGenResult.content)"></div>
                <div class="mg-result-actions">
                    <button class="btn-copy-md" @click="copyGeneratedContent">📋 복사</button>
                    <span class="mg-save-note">✅ 교안 및 학습 보조 자료에 자동 저장되었습니다</span>
                </div>
            </div>

            <!-- 생성 이력 -->
            <div v-if="matGenHistory.length > 1" class="mg-section">
                <h3 class="mg-step-title">📚 생성 이력</h3>
                <div class="mg-history-list">
                    <div v-for="(h, idx) in matGenHistory" :key="idx" class="mg-history-item" @click="matGenResult = h">
                        <span>{{ h.title }}</span>
                        <span class="mg-history-type">{{ h.material_type }}</span>
                    </div>
                </div>
            </div>
        </div>
...
.btn-stt-start:hover { filter: brightness(1.1); }
.btn-stt-stop {
...
/* 자료 생성 탭 스타일 */
/* ══════════════════════════════════════ */
.material-gen-tab-content h2 { font-size: 22px; font-weight: 700; margin-bottom: 4px; }
.tab-desc { color: #636b72; font-size: 14px; margin-bottom: 24px; }

/* 모드 전환 */
.mg-mode-toggle { display: flex; gap: 8px; margin-bottom: 20px; }
.mg-mode-btn {
    ...
    transition: all 0.2s; color: #475569; text-align: center;
}
.mg-mode-btn:hover { border-color: #6366f1; color: #6366f1; }
.mg-mode-btn.active {
...
/* 녹음 영역 */
.mg-record-area { text-align: center; padding: 10px 0; }
.mg-record-controls { display: flex; align-items: center; justify-content: center; gap: 16px; flex-wrap: wrap; }
.mg-mic-btn {
...
.mg-mic-btn:hover { transform: scale(1.05); filter: brightness(1.1); }
.mg-mic-btn.recording {
    background: linear-gradient(135deg, #ef4444, #dc2626);
    box-shadow: 0 4px 16px rgba(239,68,68,0.4);
    animation: mic-glow 1.5s ease-in-out infinite;
}
.mg-mic-icon { font-size: 22px; }
.mg-mic-pulse {
...
.mg-clear-btn:hover { background: #fee2e2; color: #ef4444; border-color: #fecaca; }

/* 녹음 인디케이터 */
.mg-recording-indicator {
    display: flex; align-items: center; justify-content: center; gap: 12px;
    margin-top: 16px; padding: 10px; background: #fef2f2; border-radius: 8px;
}
.mg-recording-label { font-size: 13px; color: #ef4444; font-weight: 500; }
.mg-wave-bars { display: flex; align-items: flex-end; gap: 3px; height: 20px; }
.mg-wave-bars span {
    display: block; width: 4px; background: #ef4444; border-radius: 2px;
    animation: wave-bar 0.8s ease-in-out infinite;
}
.mg-wave-bars span:nth-child(1) { height: 8px; animation-delay: 0s; }
.mg-wave-bars span:nth-child(2) { height: 14px; animation-delay: 0.1s; }
.mg-wave-bars span:nth-child(3) { height: 20px; animation-delay: 0.2s; }
.mg-wave-bars span:nth-child(4) { height: 14px; animation-delay: 0.3s; }
.mg-wave-bars span:nth-child(5) { height: 8px; animation-delay: 0.4s; }
@keyframes wave-bar {
    0%, 100% { transform: scaleY(0.5); }
    50% { transform: scaleY(1); }
}
.mg-record-hint {
    padding: 20px; background: #f8fafc; border: 1px dashed #cbd5e1;
    border-radius: 10px; text-align: center; font-size: 14px; color: #64748b;
...
.mg-step-title {
    font-size: 15px; font-weight: 700; color: #1a1a2e; margin: 0 0 14px 0;
    padding-bottom: 10px; border-bottom: 1px solid #f0f0f0;
}

/* 세션 목록 */
.mg-session-list { display: flex; flex-direction: column; gap: 8px; }
.mg-session-card {
    display: flex; justify-content: space-between; align-items: center;
    padding: 12px 16px; border: 1px solid #e8ecf0; border-radius: 8px;
    cursor: pointer; transition: all 0.2s;
}
.mg-session-card:hover { border-color: #6366f1; background: #f5f3ff; }
.mg-session-card.selected { border-color: #6366f1; background: #ede9fe; box-shadow: 0 0 0 2px rgba(99,102,241,0.2); }
.mg-session-info { display: flex; flex-direction: column; gap: 4px; }
.mg-session-title { font-weight: 600; font-size: 14px; color: #1a1a2e; }
.mg-session-meta { font-size: 12px; color: #636b72; display: flex; align-items: center; gap: 6px; }
.mg-session-date { font-size: 12px; color: #94a3b8; }
.mg-status { padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 600; }
.mg-status.ended { background: #f1f5f9; color: #64748b; }
.mg-status.live { background: #dcfce7; color: #16a34a; }
.mg-status.waiting { background: #fef3c7; color: #d97706; }

/* 녹취록 미리보기 */
.mg-transcript-box { border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; }
.mg-transcript-header {
    display: flex; justify-content: space-between; align-items: center;
    padding: 10px 14px; background: #f8fafc; font-size: 13px; font-weight: 600; color: #475569;
}
.mg-transcript-len { font-weight: 400; color: #94a3b8; font-size: 12px; }
.mg-transcript-text {
    padding: 14px; font-family: 'Pretendard', sans-serif; font-size: 13px;
    line-height: 1.7; color: #333; max-height: 300px; overflow-y: auto;
    white-space: pre-wrap; word-break: break-word; margin: 0; background: #fff;
}

/* 자료 유형 선택 */
.mg-type-selector { display: flex; gap: 10px; margin-bottom: 14px; flex-wrap: wrap; }
.mg-type-btn {
    padding: 10px 18px; border: 2px solid #e2e8f0; border-radius: 10px;
    background: #fff; font-size: 13px; font-weight: 600; cursor: pointer;
    transition: all 0.2s; color: #475569;
}
.mg-type-btn:hover { border-color: #6366f1; color: #6366f1; }
.mg-type-btn.active {
    border-color: #6366f1; background: linear-gradient(135deg, #6366f1, #8b5cf6);
    color: white; box-shadow: 0 2px 8px rgba(99,102,241,0.3);
}
.mg-type-desc {
    padding: 10px 14px; background: #f5f3ff; border-radius: 8px;
    font-size: 13px; color: #6366f1; margin-bottom: 14px; line-height: 1.5;
}

/* 추가 프롬프트 */
.mg-custom-prompt { margin-bottom: 16px; }
.mg-custom-prompt label { display: block; font-size: 13px; font-weight: 600; color: #475569; margin-bottom: 6px; }
.mg-custom-prompt textarea {
    width: 100%; padding: 10px 12px; border: 1px solid #e2e8f0; border-radius: 8px;
    font-size: 13px; resize: vertical; font-family: inherit; outline: none; box-sizing: border-box;
}
.mg-custom-prompt textarea:focus { border-color: #6366f1; box-shadow: 0 0 0 2px rgba(99,102,241,0.15); }
...
.mg-generate-btn:hover:not(:disabled) { filter: brightness(1.1); transform: translateY(-1px); }
.mg-generate-btn:disabled { opacity: 0.7; cursor: wait; }
.spinner-sm {
...
/* 생성 결과 */
.mg-result { border-color: #6366f1; background: #fafbff; }
.mg-result-header {
    display: flex; justify-content: space-between; align-items: center;
    margin-bottom: 14px; flex-wrap: wrap; gap: 8px;
}
.mg-result-title { font-weight: 700; font-size: 15px; color: #1a1a2e; }
.mg-result-badge {
    padding: 4px 10px; background: #ede9fe; color: #6366f1;
    border-radius: 6px; font-size: 11px; font-weight: 600;
}
.mg-result-content {
    padding: 20px; background: #fff; border: 1px solid #e8ecf0;
    border-radius: 8px; font-size: 14px; line-height: 1.8; color: #333;
    max-height: 600px; overflow-y: auto;
}
.mg-result-content h1, .mg-result-content h2, .mg-result-content h3 { color: #1a1a2e; margin-top: 20px; }
.mg-result-content table { border-collapse: collapse; width: 100%; margin: 12px 0; }
.mg-result-content th, .mg-result-content td { border: 1px solid #e2e8f0; padding: 8px 12px; text-align: left; font-size: 13px; }
.mg-result-content th { background: #f8fafc; font-weight: 600; }
.mg-result-content code { background: #f1f5f9; padding: 2px 6px; border-radius: 4px; font-size: 12px; }
.mg-result-content ul, .mg-result-content ol { padding-left: 24px; }
.mg-result-actions { display: flex; align-items: center; gap: 12px; margin-top: 14px; }
.btn-copy-md {
    padding: 8px 16px; background: #6366f1; color: white; border: none;
    border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer;
}
.btn-copy-md:hover { background: #4f46e5; }
.mg-save-note { font-size: 12px; color: #16a34a; font-weight: 500; }

/* 이력 */
.mg-history-list { display: flex; flex-direction: column; gap: 6px; }
.mg-history-item {
    display: flex; justify-content: space-between; align-items: center;
    padding: 10px 14px; border: 1px solid #e8ecf0; border-radius: 8px;
    cursor: pointer; font-size: 13px; transition: background 0.2s;
}
.mg-history-item:hover { background: #f5f3ff; }
.mg-history-type {
    padding: 2px 8px; background: #ede9fe; color: #6366f1;
    border-radius: 6px; font-size: 11px; font-weight: 600;
}
</style>
