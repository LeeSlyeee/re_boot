...
class LiveSessionViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=['get'], url_path='stt-transcript')
    def stt_transcript(self, request, pk=None):
        """
        GET /api/learning/live/{id}/stt-transcript/
        교수자: 해당 세션의 전체 STT 녹취록 조회
        """
        session = get_object_or_404(LiveSession, id=pk, instructor=request.user)
        chunks = session.stt_logs.order_by('sequence_order').values_list('text_chunk', flat=True)
        transcript = '\n'.join(chunks)
        return Response({
            'session_id': session.id,
            'title': session.title or (session.lecture.title if session.lecture else ''),
            'status': session.status,
            'chunk_count': len(chunks),
            'transcript': transcript,
        })

    @action(detail=True, methods=['post'], url_path='generate-material')
    def generate_material(self, request, pk=None):
        """
        POST /api/learning/live/{id}/generate-material/
        교수자: STT 녹취록 기반으로 AI가 강의 자료를 생성하고 LectureMaterial로 저장
        
        body: {
            "material_type": "summary" | "handout" | "review",  (default: "summary")
            "custom_prompt": "(선택) 추가 지시사항"
        }
        """
        session = get_object_or_404(LiveSession, id=pk, instructor=request.user)

        # STT 녹취록 수집
        chunks = list(session.stt_logs.order_by('sequence_order').values_list('text_chunk', flat=True))
        if not chunks:
            return Response(
                {'error': 'STT 녹취록이 없습니다. 먼저 라이브 세션에서 STT를 사용해주세요.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        transcript = '\n'.join(chunks)

        material_type = request.data.get('material_type', 'summary')
        ...
        type_prompts = {
            'summary': {
                'name': '강의 요약 노트',
                'instruction': '''아래 강의 녹취록을 분석하여 **체계적인 강의 요약 노트**를 마크다운 형식으로 작성해주세요.

포함할 내용:
1. **강의 개요** (한 문단으로 핵심 주제 요약)
2. **핵심 개념** (번호 매기기, 각 개념에 대한 간결한 설명)
3. **주요 키워드** (표 형식: 키워드 | 정의)
4. **수업 흐름 정리** (시간 순서대로 다룬 내용)
5. **핵심 정리** (5~7개 항목, 불릿 포인트)'''
            },
            'handout': {
                'name': '학습용 핸드아웃',
                'instruction': '''아래 강의 녹취록을 분석하여 **학생 배포용 핸드아웃**을 마크다운 형식으로 작성해주세요.

포함할 내용:
1. **학습 목표** (이 강의를 듣고 나면 할 수 있는 것들)
2. **핵심 내용 정리** (학생이 이해하기 쉽게 구조화)
3. **중요 용어 사전** (표 형식: 용어 | 설명 | 예시)
4. **연습 문제** (이해도 점검용 3~5문제, 답안 포함)
5. **추가 학습 가이드** (심화 학습을 위한 추천 주제)'''
            },
            'review': {
                'name': '복습 가이드',
                'instruction': '''아래 강의 녹취록을 분석하여 **시험/복습용 가이드**를 마크다운 형식으로 작성해주세요.

포함할 내용:
1. **핵심 개념 체크리스트** (각 항목에 ☐ 체크박스)
2. **자주 헷갈리는 개념 비교** (표 형식으로 비교)
3. **기출 예상 문제** (5~8문제, 주관식/객관식 혼합, 정답+해설 포함)
4. **한 줄 요약** (각 주제별 핵심을 한 줄로)
5. **오답 노트 포인트** (학생들이 틀리기 쉬운 포인트)'''
            },
        }

        type_info = type_prompts.get(material_type, type_prompts['summary'])
        lecture_title = session.lecture.title if session.lecture else '강의'

        system_prompt = f"""당신은 대학 교수를 돕는 AI 강의 자료 생성 전문가입니다.
강의 녹취록을 분석하여 고품질의 학습 자료를 생성합니다.
반드시 한국어로 작성하며, 마크다운 형식을 사용합니다.
전문적이면서도 학생이 이해하기 쉬운 톤으로 작성합니다."""

        user_prompt = f"""{type_info['instruction']}

{f'추가 지시사항: {custom_prompt}' if custom_prompt else ''}

---
**강의명**: {lecture_title}
**세션**: {session.title or '라이브 세션'}
...
"""
        ...
        try:
            ...
            logger.error(f"❌ [GenerateMaterial] OpenAI 호출 실패: {str(e)}")
            return Response(
                {'error': f'AI 자료 생성 실패: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

        # LectureMaterial로 저장 (MARKDOWN 타입)
        material_title = f"[AI] {type_info['name']} - {session.title or lecture_title} ({timezone.now().strftime('%m/%d')})"
        ...
        logger.info(f"✅ [GenerateMaterial] 자료 생성 완료: material_id={material.id}, type={material_type}")
...
class LectureMaterialViewSet(viewsets.ViewSet):
    ...
    def generate_from_text(self, request):
        """
        POST /api/learning/materials/generate-from-text/
        교수자: 직접 입력한 STT 녹취 텍스트로 AI 강의 자료 생성
        
        body: {
            "lecture_id": 1,
            "transcript": "강의 녹취 텍스트...",
            "material_type": "summary" | "handout" | "review",
            "custom_prompt": "(선택) 추가 지시사항"
        }
        """
        lecture_id = request.data.get('lecture_id')
...
5. **오답 노트 포인트** (학생들이 틀리기 쉬운 포인트)'''
        ...
        system_prompt = """당신은 대학 교수를 돕는 AI 강의 자료 생성 전문가입니다.
강의 녹취록을 분석하여 고품질의 학습 자료를 생성합니다.
반드시 한국어로 작성하며, 마크다운 형식을 사용합니다.
전문적이면서도 학생이 이해하기 쉬운 톤으로 작성합니다."""
...
"""
        ...
        try:
            ...
            logger.error(f"❌ [GenerateFromText] OpenAI 호출 실패: {str(e)}")
            return Response(
                {'error': f'AI 자료 생성 실패: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

        material_title = f"[AI] {type_info['name']} - {lecture_title} ({timezone.now().strftime('%m/%d %H:%M')})"
        material = LectureMaterial.objects.create(
            lecture=lecture,
            title=material_title,
            content_type='MARKDOWN',
        ...
        logger.info(f"✅ [GenerateFromText] 자료 생성 완료: material_id={material.id}, type={material_type}")
...
def _generate_quiz_suggestion(session_id):
    ...
    try:
        ...
        if not recent_chunks:
            # STT 없음 fallback: 교안 또는 강의 제목 기반 퀴즈 생성
            # 1순위: 교안 텍스트 기반
            ...
            if material_text:
                ...
                logger.info(f"📝 [Fallback] STT 없음 → 교안 기반 퀴즈 생성 (세션 #{session_id})")
            else:
                ...
                logger.info(f"📝 [Fallback] STT·교안 모두 없음 → 강의 제목 기반 퀴즈 생성 (세션 #{session_id}, 강의: {lecture_title})")
        else:
